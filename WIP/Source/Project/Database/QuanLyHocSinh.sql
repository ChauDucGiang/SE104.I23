...
GO
ALTER PROC USP_UpdateHocSinh
@mahs INT, @tenhs NVARCHAR(100), @ngaysinh DATETIME, @noisinh NVARCHAR(100), @sodienthoai NVARCHAR(100), @diachi NVARCHAR(100), @gioitinh NVARCHAR(100), @email NVARCHAR(100)
AS
BEGIN
...
END
GO
EXEC dbo.USP_UpdateHocSinh @mahs = 0,                         -- int
                           @tenhs = N'',                      -- nvarchar(100)
                           @ngaysinh = '2018-04-18 03:53:39', -- datetime
                           @noisinh = N'',                    -- nvarchar(100)
                           @sodienthoai = N'',                -- nvarchar(100)
                           @diachi = N'',                     -- nvarchar(100)
                           @gioitinh = N'',                   -- nvarchar(100)
                           @email = N''                       -- nvarchar(100)

    

GO	
						   	UPDATE dbo.HocSinh SET	
									HoTen=N'Phạm Thanh Đức Cống'
							WHERE MaHS=1

GO
EXEC dbo.USP_ThemHocSinh @mahs = 0,                -- int
                         @hoten = N'',             -- nvarchar(100)
                         @gioitinh = N'',          -- nvarchar(100)
                         @ngaysinh = '2018-04-18', -- date
                         @noisinh = N'',           -- nvarchar(100)
                         @dienthoai = N'',         -- nvarchar(100)
                         @email = N'',             -- nvarchar(100)
                         @diachi = N''             -- nvarchar(100)
...
AS
	...
	END
GO	
EXEC dbo.USP_XoaHocSinh @mahs = 1 -- int
