...
namespace QLHS.DAO
{
    public class GiaoVienDAO
    {
        ...
        public List<GiaoVien> SearchGVByMaGV(string magv)
        {
            ...
            string query = "SELECT * FROM GiaoVien WHERE MaGV LIKE N'"+magv+"'";
        ...
        {
            List<GiaoVien> list = new List<GiaoVien>();
            string query = "SELECT * FROM dbo.GiaoVien WHERE dbo.fuConvertToUnsign1(TenGV) LIKE N'%'+dbo.fuConvertToUnsign1(N'"+tengv+"')+'%'";
            DataTable data = DataProvider.Instance.ExecuteQuery(query);
            foreach (DataRow r in data.Rows)
            {
                GiaoVien gv = new GiaoVien(r);
