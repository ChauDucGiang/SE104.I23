...
namespace QLHS.DAO
{
    public class AccountDAO
    {
        ...
        {
            string query = "SELECT TenNguoiDung, Type, UserName , PassWord FROM dbo.Account";
            DataTable data = DataProvider.Instance.ExecuteQuery(query);
            return data;
        }
        public bool AddAccount(string username, string password, string tennguoidung, string loai)
        {
            string query = "EXEC dbo.USP_ThemAccount @username = N'" + username + "' , @password = N'" + password + "' , @name = N'" + tennguoidung + "' , @loai = N'" + loai + "'";
            int re = DataProvider.Instance.ExecuteNonQuery(query);
            return re > 0;
        }
